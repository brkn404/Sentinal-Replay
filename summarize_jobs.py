...
def export_summary_to_csv(jobs, output_file):
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "Job ID", "Start Time", "End Time", "Duration (sec)", "Duration (min)",
            "Files", "Data (GB)", "Raw Bytes", "Avg MB/s"
        ])
        for jobid in sorted(jobs):
            s = analyze_job(jobid, jobs[jobid])
            writer.writerow([
                s["jobid"], s["start"], s["end"], s["duration_sec"],
                round(s["duration_min"], 2), s["files"], round(s["data_gb"], 2),
                s["raw_bytes"], round(s["avg_MBps"], 3)
            ])
...
def main():
    ...
    export_summary_to_csv(jobs, "job_summary.csv")
